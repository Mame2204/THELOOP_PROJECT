...
<body>
  ...
  <script>
    (function () {
      ...
      if (prebootSession && prebootSession.verifyError) {
        title.textContent = 'Lien invalide ou expiré';
        message.textContent = prebootSession.verifyError;
        return;
      }
      if (prebootSession && prebootSession.accessToken && prebootSession.refreshToken) {
        var pk = (prebootSession.kind || 'invite').toLowerCase();
        var pps =
          'type=' + pk
          + '&access_token=' + encodeURIComponent(prebootSession.accessToken)
          + '&refresh_token=' + encodeURIComponent(prebootSession.refreshToken);
        beginSession(prebootSession.accessToken, prebootSession.refreshToken, pk, pps);
        return;
      }
      ...
      var stuckGuard = window.setTimeout(function () {
        if (title.textContent === 'Chargement…') {
          ...
          message.textContent =
            'Ouvrez le lien dans Safari ou Chrome (icône ⋯ du mail → « Ouvrir dans le navigateur »). Si le problème continue, demandez un nouvel e-mail d\\\\'invitation.';
        }
      }, 8000);
