-- THE LOOP — Fallback RPC validation partenaire SANS colonnes content_* sur benefit_redemptions
-- Utiliser si apply_benefit_redemption_content_columns échoue (42501 owner).
-- Sécurité : SECURITY DEFINER, search_path fixe, code partenaire obligatoire, statuts contrôlés.
...
BEGIN
  ...
  FOR v_red IN
    SELECT br.local_id, br.benefit_id, br.user_id
    FROM public.benefit_redemptions br
