-- THE LOOP — Clics événements + tarif « sur invitation »

ALTER TABLE public.events
  ADD COLUMN IF NOT EXISTS is_invitation_only BOOLEAN NOT NULL DEFAULT FALSE;

ALTER TABLE public.partner_event_submissions
  ADD COLUMN IF NOT EXISTS is_invitation_only BOOLEAN NOT NULL DEFAULT FALSE;

COMMENT ON COLUMN public.events.is_invitation_only IS
  'Entrée sur invitation uniquement (distinct de gratuit).';

-- ─── RPC : incrémenter un clic événement ────────────────────────────────────
...
-- ─── Admin : création directe (is_invitation_only) ──────────────────────────
