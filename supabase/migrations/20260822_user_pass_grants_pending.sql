-- PASS en file d'attente : statut pending + métadonnées achat (sync multi-appareil)
...
ALTER TABLE public.user_pass_grants
  ADD COLUMN IF NOT EXISTS amount_gnf INTEGER,
...
COMMENT ON COLUMN public.user_pass_grants.status IS
  'active | pending (file d''attente) | suspended (gel admin) | expired | revoked';

-- Une seule signature : supprimer l’ancienne version (20260821, 8 args) avant la nouvelle (13 args)
...
BEGIN
  ...
  -- Promotion Prime uniquement pour un PASS actif (pas pending / suspended)
...
COMMENT ON FUNCTION public.upsert_user_pass_purchase(
  ...
  'Sync achat PASS (actif ou file d''attente) — promotion Prime seulement si status=active.';
