-- Origine de création des événements : admin (Control Tower), loop (onglet THE LOOP), partner.
ALTER TABLE public.events
  ADD COLUMN IF NOT EXISTS content_origin TEXT;

COMMENT ON COLUMN public.events.content_origin IS 'Canal de création : admin, loop, partner.';
...
-- Soumissions partenaire : conserver l'origine pour la publication.
ALTER TABLE public.partner_event_submissions
  ADD COLUMN IF NOT EXISTS content_origin TEXT;

COMMENT ON COLUMN public.partner_event_submissions.content_origin IS 'Canal de création : admin, loop, partner.';
